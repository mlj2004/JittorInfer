...
MINI_API struct llama_sampler * llama_sampler_init_grammar(
...
MINI_API struct llama_sampler * llama_sampler_init_dry(
        const struct llama_vocab *  vocab,
                            int32_t    n_ctx_train,
...
MINI_API struct llama_sampler * llama_sampler_init_logit_bias(
                            int32_t   n_vocab,
...
MINI_API int32_t llama_tokenize(
    const struct llama_vocab * vocab,
                    const char * text,
                        int32_t   text_len,
                    llama_token * tokens,
...
MINI_API int32_t llama_token_to_piece(
            const struct llama_vocab * vocab,
                        llama_token   token,
                                char * buf,
                            int32_t   length,
...
MINI_API int32_t llama_detokenize(
    const struct llama_vocab * vocab,
...
// p0 < 0     : [0,  p1]
// p1 < 0     : [p0, inf)
MINI_API bool llama_kv_cache_seq_rm(
...
// p0 < 0 : [0,  p1]
// p1 < 0 : [p0, inf)
MINI_API void llama_kv_cache_seq_cp(
        struct llama_context * ctx,
...
// p0 < 0 : [0,  p1]
// p1 < 0 : [p0, inf)
MINI_API void llama_kv_cache_seq_add(
        struct llama_context * ctx,
                llama_seq_id   seq_id,
                    llama_pos   p0,
                    llama_pos   p1,
...
//
MINI_API struct llama_batch llama_batch_get_one(
                llama_token * tokens,
                    int32_t   n_tokens);
...
MINI_API struct llama_batch llama_batch_init(
        int32_t n_tokens,
        int32_t embd,
        int32_t n_seq_max);
...
MINI_API int32_t llama_chat_apply_template(
                        const char * tmpl,
