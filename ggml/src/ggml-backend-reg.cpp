...
struct dl_handle_deleter {
    void operator()(HMODULE handle) { FreeLibrary(handle); }
};
        ...
        if (!reg || reg->api_version != GGML_BACKEND_API_VERSION) {
            if (!silent) {
                if (!reg) {
                    ...
                    GGML_LOG_ERROR(
                        "%s: failed to initialize backend from %s: incompatible API version (backend: %d, current: "
                        "%d)\n",
...
static ggml_backend_reg_t ggml_backend_load_best(const char * name, bool silent, const char * user_search_path) {
    ...
    if (best_score == 0) {
        ...
        for (const auto & search_path : search_paths) {
            fs::path filename =
                backend_filename_prefix().native() + name_path.native() + backend_filename_extension().native();
