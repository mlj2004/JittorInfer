...
static void ggml_dyn_tallocr_reset(struct ggml_dyn_tallocr * alloc) {
    ...
    alloc->free_blocks[0].size =
        SIZE_MAX / 2;  // restrict maximum size of a measure allocator to half size_t max to avoid overflows
    alloc->max_size = 0;
...
static void ggml_gallocr_init_tensor(ggml_gallocr_t galloc, struct ggml_tensor * tensor,
    ...
    assert(tensor->data || tensor->view_src ||
           ggml_backend_buffer_get_alloc_size(galloc->buffers[buffer_id], tensor) <= tensor_alloc->size_max);
...
static bool alloc_tensor_range(struct ggml_context * ctx, struct ggml_tensor * first, struct ggml_tensor * last,
