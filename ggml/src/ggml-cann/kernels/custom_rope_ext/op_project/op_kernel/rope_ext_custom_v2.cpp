...
class KernelRopeV2 {
   public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR cos, GM_ADDR sin, GM_ADDR dst,
                                int32_t ne0, int32_t ne1, int32_t pos_len) {
        ...
        cosGm.SetGlobalBuffer((__gm__ float *)cos + curr_pos * this->blockLength, this->blockLength);
        sinGm.SetGlobalBuffer((__gm__ float *)sin + curr_pos * this->blockLength, this->blockLength);
        dstGm.SetGlobalBuffer((__gm__ T *)dst + blockIndex * this->ne0, this->ne0);
        
        pipe.InitBuffer(inQueueX, BUFFER_NUM, this->ne0 * sizeof(T));
        pipe.InitBuffer(inQueueCos, BUFFER_NUM, this->blockLength * sizeof(float));
        pipe.InitBuffer(inQueueSin, BUFFER_NUM, this->blockLength * sizeof(float));
   ...
   private:
    ...
    __aicore__ inline void ComputeRope(int32_t row) {
        ...
        if constexpr (std::is_same<T, float>::value) {
            DataCopy(x0, xLocal, blockN); // x0 = first blockN elements of xLocal
            DataCopy(x1, xLocal[blockN], blockN); // x1 = second blockN elements of xLocal
        } else {
            // For half precision, cast xLocal to float before copying.
            Cast(x0, xLocal, RoundMode::CAST_NONE, blockN); // x0 = cast(xLocal) for first blockN elements
            Cast(x1, xLocal[blockN], RoundMode::CAST_NONE, blockN); // x1 = cast(xLocal) for second blockN elements
        }
        ...
        Sub(tmp0, tmp0, tmp1, blockN); // tmp0 = (x0 * cos_theta) - (x1 * sin_theta)

        if constexpr (std::is_same<T, float>::value)
            DataCopy(dstLocal, tmp0, blockN); // dstLocal[0:blockN] = tmp0 (first half)
        else
            Cast(dstLocal, tmp0, RoundMode::CAST_NONE, blockN); // For half precision, cast result
        ...
        Add(tmp0, tmp0, tmp1, blockN); // tmp0 = (x0 * sin_theta) + (x1 * cos_theta)

        if constexpr (std::is_same<T, float>::value)
            DataCopy(dstLocal[blockN], tmp0, blockN); // dstLocal[blockN:2*blockN] = tmp0 (second half)
        else
            Cast(dstLocal[blockN], tmp0, RoundMode::CAST_NONE, blockN); // For half precision, cast result
...
extern "C" __global__ __aicore__ void rope_ext_custom_v2(
    GM_ADDR x, GM_ADDR cos, GM_ADDR sin, GM_ADDR dst, GM_ADDR workspace, GM_ADDR tiling) {
    GET_TILING_DATA(tiling_data, tiling);
    KernelRopeV2<DTYPE_X> op;
    op.Init(x, cos, sin, dst, tiling_data.ne0, tiling_data.ne1, tiling_data.pos_len);
    op.Process();
}
