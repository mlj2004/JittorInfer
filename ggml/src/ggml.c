...
void ggml_fp32_to_i32_row(const float * x, int * y, int64_t n) {
    for (int64_t i = 0; i < n; i++) {
        y[i] = (int) x[i];
    }
}
...
struct ggml_tensor * ggml_rms_norm_fused(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * w,
    ...
    result->op     = GGML_OP_RMS_NORM_FUSED;
    result->src[0] = a;
    result->src[1] = w;
...
struct ggml_tensor * ggml_dpskv2_fused_moe(struct ggml_context * ctx, struct ggml_tensor * inp_embd,
                                           struct ggml_tensor * ids, struct ggml_tensor * weight_gate,
                                           struct ggml_tensor * weight_up, struct ggml_tensor * weight_down,
                                           int32_t start_expert, int32_t end_expert, int32_t n_expert_groups) {
    GGML_ASSERT(ggml_is_contiguous(inp_embd));
    GGML_ASSERT(ids->type == GGML_TYPE_I32);

    GGML_ASSERT(weight_gate->ne[3] == 1);                // as is 3d (one matrix per expert)
    GGML_ASSERT(weight_up->ne[3] == 1);                  // as is 3d (one matrix per expert)
    GGML_ASSERT(weight_down->ne[3] == 1);                // as is 3d (one matrix per expert)
    GGML_ASSERT(inp_embd->ne[3] == 1);                   // as is 3d (one matrix per expert)
    GGML_ASSERT(ids->ne[2] == 1 && ids->ne[3] == 1);     // ids is 2d
    GGML_ASSERT(ids->ne[1] == inp_embd->ne[2]);          // must have an expert list per b row
    GGML_ASSERT(inp_embd->ne[0] == weight_gate->ne[0]);  // can_mul_mat
    GGML_ASSERT(inp_embd->ne[0] == weight_up->ne[0]);    // can_mul_mat
    GGML_ASSERT(ids->ne[0] % inp_embd->ne[1] == 0);      // can broadcast
    // ep: 范围
    GGML_ASSERT(start_expert < end_expert);
    GGML_ASSERT(0 <= start_expert && start_expert < (int32_t) weight_gate->ne[2] * n_expert_groups);
    GGML_ASSERT(0 < end_expert && end_expert <= (int32_t) weight_gate->ne[2] * n_expert_groups);

    const int64_t        ne[4]  = { weight_down->ne[1], ids->ne[0], inp_embd->ne[2], 1 };
    struct ggml_tensor * result = ggml_new_tensor(ctx, GGML_TYPE_F32, 4, ne);

    result->op           = GGML_OP_DPSKV2_FUSED_MOE;
    result->src[0]       = inp_embd;
    result->src[1]       = ids;
    result->src[2]       = weight_gate;
    result->src[3]       = weight_up;
    result->src[4]       = weight_down;
    result->op_params[0] = start_expert;
    result->op_params[1] = end_expert;
    result->op_params[2] = n_expert_groups;
    return result;
}

struct ggml_tensor * ggml_moe_fused(struct ggml_context * ctx, struct ggml_tensor * input, struct ggml_tensor * ids,
    ...
    result->src[0]              = input;
    result->src[1]              = ids;
    result->src[2]              = topk_weights;
    result->src[3]              = expert_up_weights;
    result->src[4]              = expert_down_weights;
    result->src[5]              = expert_gate_weights;
    result->src[6]              = row_idx_permute;

    result->op_params[0] = start_idx;
    result->op_params[1] = end_idx;
    return result;
}
...
struct ggml_tensor * ggml_moe_fused_cpu(struct ggml_context * ctx, struct ggml_tensor * input)
// struct ggml_tensor * ids, struct ggml_tensor * topk_weights,
// struct ggml_tensor * expert_up_weights,
// struct ggml_tensor * expert_down_weights){
{
    // struct ggml_tensor * result = ggml_new_tensor(ctx, GGML_TYPE_F32, 4, input->ne);
    // result->op     = GGML_OP_MOE_FUSED_CPU;
    // result->src[0] = input;
    // result->src[1] = ids;
    // result->src[2] = topk_weights;
    // result->src[3] = expert_up_weights;
...
struct ggml_tensor * ggml_flash_attn_prompt(struct ggml_context * ctx, struct ggml_tensor * query,
                                            ...
                                            float scaleValue) {
    // const int64_t        ne[4]  = { batch_size, num_heads, sequence_lenth_q, head_dim_v };
    const int64_t        ne[4]  = { head_dim_v, num_heads, sequence_lenth_q, batch_size };
    //TODO: check the type
    struct ggml_tensor * result = ggml_new_tensor(ctx, GGML_TYPE_F16, 4, ne);
    result->op                  = GGML_OP_FLASH_ATTN_PROMPT;
    result->src[0]              = query;
    result->src[1]              = key;
    result->src[2]              = value;
    result->src[3]              = attn_mask;
    // these tensors are valid for only ge.
...
struct ggml_tensor * ggml_get_slice(struct ggml_context * ctx, struct ggml_tensor * a, int64_t fr, int64_t to,
                                    int axis) {
    GGML_ASSERT(fr < to);
    GGML_ASSERT(axis >= 0 && axis < GGML_MAX_DIMS);
    GGML_ASSERT(a->ne[axis] >= to);
    ...
    result->op     = GGML_OP_GET_SLICE;
    result->src[0] = a;

    int64_t params[] = { fr, to, axis };
    ggml_set_op_params(result, params, sizeof(params));
...
struct ggml_tensor * ggml_scatter_update(struct ggml_context * ctx, struct ggml_tensor * var,
                                         struct ggml_tensor * indices, struct ggml_tensor * updates) {
    struct ggml_tensor * result = ggml_view_tensor(ctx, var);
    result->op                  = GGML_OP_SCATTER_UPDATE;
    result->src[0]              = var;
    result->src[1]              = indices;
    result->src[2]              = updates;

    GGML_ASSERT(indices->type == GGML_TYPE_I32 || indices->type == GGML_TYPE_I64);

    GGML_ASSERT(var->ne[0] == updates->ne[0]);
    GGML_ASSERT(var->ne[2] == updates->ne[2] == 1);
    GGML_ASSERT(var->ne[3] == updates->ne[3] == 1);
    GGML_ASSERT(indices->ne[0] == updates->ne[1]);
    GGML_ASSERT(indices->ne[1] == 1);
...
struct ggml_tensor * ggml_get_rows_fp16(struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b) {
    GGML_ASSERT(a->ne[2] == b->ne[1]);
    GGML_ASSERT(b->ne[3] == 1);
    GGML_ASSERT(b->type == GGML_TYPE_I32);
...
static struct ggml_tensor * ggml_map_custom2_impl_f32(struct ggml_context * ctx, struct ggml_tensor * a,
...
static struct ggml_tensor * ggml_map_custom3_impl_f32(struct ggml_context * ctx, struct ggml_tensor * a,
...
void ggml_build_backward_expand(struct ggml_context * ctx_static, struct ggml_context * ctx_compute,
                                struct ggml_cgraph * cgraph, bool accumulate) {
...
static size_t ggml_graph_nbytes(size_t size, bool grads) {
    ...
    incr_ptr_aligned(&p, 64, 1);  // 为时间戳字符串分配64字节空间
...
struct ggml_cgraph * ggml_new_graph_custom(struct ggml_context * ctx, size_t size, bool grads) {
    ...
    // 在图的内存区域内为时间戳字符串分配空间
    char * graph_name_by_time = (char *) incr_ptr_aligned(&p, 64, 1);

    // check that we allocated the correct amount of memory
    assert(obj_size == (size_t) ((char *) p - (char *) cgraph));

    // 使用高精度时间戳（微秒）并写入分配的内存空间
    if (graph_name_by_time) {
        int64_t time_us = ggml_time_us();  // 使用微秒级时间戳
        snprintf(graph_name_by_time, 64, "%lld", (long long) time_us);
    } else {
        // 这种情况不应该发生，因为内存是在图分配时预先分配的
        graph_name_by_time = "(unknown time)";
    }
    ...
    *cgraph = (struct ggml_cgraph){
        ...
        /*.graph_name_by_time =*/graph_name_by_time,
    };
...
void ggml_graph_set_flags(struct ggml_cgraph * cgraph, int flags) {
    cgraph->flags = flags;
}
...
static struct ggml_tensor * ggml_all_reduce_sum_impl(struct ggml_context * ctx, struct ggml_tensor * a) {
    struct ggml_tensor * result = ggml_dup_tensor(ctx, a);
    ggml_format_name(result, "%s (all_reduce_sum)", a->name);
...
struct ggml_tensor * ggml_all_reduce_sum(struct ggml_context * ctx, struct ggml_tensor * a) {
    return ggml_all_reduce_sum_impl(ctx, a);
}
