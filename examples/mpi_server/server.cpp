...
static llama_context_params common_context_params_to_llama_local(common_params & params) {
...
int mpi_decode(llama_context * ctx, llama_batch * batch, char * mpi_batch_data) {
    common_status comm_status = { MPI_OP_DECODE, { 0 } };
    comm_status.data[0]       = common_dump_batch(*batch, 0, mpi_batch_data);
    MPI_Bcast(&comm_status, sizeof(comm_status), MPI_UNSIGNED_LONG, 0, MPI_COMM_WORLD);
    MPI_Bcast(mpi_batch_data, comm_status.data[0], MPI_CHAR, 0, MPI_COMM_WORLD);
    return llama_decode(ctx, *batch);
}

int mpi_decode_subprocess(llama_context * ctx, llama_batch * batch, char * mpi_batch_data, common_status comm_status) {
    MPI_Bcast(mpi_batch_data, comm_status.data[0], MPI_CHAR, 0, MPI_COMM_WORLD);
    common_load_batch(*batch, 0, mpi_batch_data);
    return llama_decode(ctx, *batch);
}

bool mpi_kv_rm_process(llama_context * ctx, llama_seq_id seq_id, llama_pos p0, llama_pos p1) {
    common_status comm_status = {
        MPI_OP_KV_RM, { 0, 0, 0 }
    };
    comm_status.data[0] = seq_id;
    comm_status.data[1] = p0;
    comm_status.data[2] = p1;
    MPI_Bcast(&comm_status, sizeof(comm_status), MPI_UNSIGNED_LONG, 0, MPI_COMM_WORLD);
    return llama_kv_cache_seq_rm(ctx, comm_status.data[0], comm_status.data[1], comm_status.data[2]);
}

void mpi_kv_rm_subprocess(llama_context * ctx, common_status comm_status) {
    llama_kv_cache_seq_rm(ctx, comm_status.data[0], comm_status.data[1], comm_status.data[2]);
}

void mpi_kv_add_process(llama_context * ctx, llama_seq_id seq_id, llama_pos p0, llama_pos p1, llama_pos delta) {
    common_status comm_status = {
        MPI_OP_KV_ADD, { 0, 0, 0, 0 }
    };
    comm_status.data[0] = seq_id;
    comm_status.data[1] = p0;
    comm_status.data[2] = p1;
    comm_status.data[3] = delta;
    MPI_Bcast(&comm_status, sizeof(comm_status), MPI_UNSIGNED_LONG, 0, MPI_COMM_WORLD);
    llama_kv_cache_seq_add(ctx, comm_status.data[0], comm_status.data[1], comm_status.data[2], comm_status.data[3]);
}

void mpi_kv_add_subprocess(llama_context * ctx, common_status comm_status) {
    llama_kv_cache_seq_add(ctx, comm_status.data[0], comm_status.data[1], comm_status.data[2], comm_status.data[3]);
}

static common_init_result common_init_from_params_local(common_params & params) {
    common_init_result iparams;
    auto               mparams  = common_model_params_to_llama_local(params);
    llama_model *      model    = nullptr;
    model                       = llama_model_load_from_file(params.model.c_str(), mparams);
    const llama_vocab * vocab   = llama_model_get_vocab(model);
    auto                cparams = common_context_params_to_llama_local(params);
    llama_context *     lctx    = llama_init_from_model(model, cparams);

    if (lctx == NULL) {
        std::cerr << __func__ << ": failed to create context with model '" << params.model << "'\n";
...
struct server_context {
    ...
    bool load_model(const common_params & params) {
        params_base = params;
        llama_init  = common_init_from_params_local(params_base);
    ...
    void init() {
        ...
        }
        {
            ...
            mpi_batch_data        = (char *) malloc(common_llama_batch_max_comm_size(n_ctx, 0, 1));
        }
    }
    ...
    void update_slots() {
        ...
        for (server_slot & slot : slots) {
            if (slot.is_processing() && slot.n_past + 1 >= slot.n_ctx) {
                ...
                mpi_kv_rm_process(ctx, slot.id, n_keep, n_keep + n_discard);
                mpi_kv_add_process(ctx, slot.id, n_keep + n_discard, slot.n_past, -n_discard);
        ...
        if (params_base.cont_batching || batch.n_tokens == 0) {
            for (auto & slot : slots) {
                ...
                if (slot.state == SLOT_STATE_PROCESSING_PROMPT || slot.state == SLOT_STATE_STARTED) {
                    ...
                    if (!mpi_kv_rm_process(ctx, slot.id, slot.n_past, -1)) {
        ...
        for (int32_t i = 0; i < batch.n_tokens; i += n_batch) {
            ...
            const int ret = mpi_decode(ctx, &batch_view, mpi_batch_data);
...
int main(int argc, char ** argv) {
    ...
    // 初始化MPI环境
    MPI_Init(&argc, &argv);
    MPI_Comm_rank(MPI_COMM_WORLD, &ctx_server.mpi_rank);
#endif
    ...
    if (ctx_server.mpi_rank == 0) {
        ...
        const auto handle_completions_impl = [&ctx_server, &res_ok](server_task_type type, json & data,
            ...
            if (!stream) {
                ...
                const auto chunked_content_provider = [task_ids, &ctx_server, oaicompat](size_t,
        ...
        const auto handle_chat_completions = [&ctx_server, &params, &handle_completions_impl](
        ...
        while (true) {
            common_status comm_status;
            MPI_Bcast(&comm_status, sizeof(comm_status), MPI_UNSIGNED_LONG, 0, MPI_COMM_WORLD);
            switch (comm_status.op) {
                case MPI_OP_DECODE:
                    mpi_decode_subprocess(ctx_server.ctx, &ctx_server.batch, ctx_server.mpi_batch_data, comm_status);
                    break;
                case MPI_OP_KV_ADD:
                    mpi_kv_add_subprocess(ctx_server.ctx, comm_status);
                    break;
                case MPI_OP_KV_RM:
                    mpi_kv_rm_subprocess(ctx_server.ctx, comm_status);
                    break;
            }
        }
    }
